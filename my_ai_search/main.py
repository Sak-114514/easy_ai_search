...
def _prefilter_search_results(
    ...
    fetch_target: Optional[int] = None,
...
def _validate_and_prepare(query: str, max_results: Optional[int], disable_deep_process: bool, mode: str, client_type: str) -> Dict[str, Any]:
    config = get_config()
    ...
    try:
        ...
        if not skip_local:
            ...
            if len(high_quality_local_results) >= actual_max_results:
                ...
                logger.info(
                    f"Found only {len(high_quality_local_results)} high-quality results in local DB, proceeding to online search"
                )
        ...
        intent_plan = online_phase["intent_plan"]
